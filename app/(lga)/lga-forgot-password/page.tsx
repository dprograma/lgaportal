...
import { Mail, ArrowLeft, BadgeCheck, CheckCircle2, Building2 } from "lucide-react";

import { forgotPasswordSchema, type ForgotPasswordInput } from "@/lib/validations";
import { Input } from "@/components/ui/input";
import Button from "@/components/ui/button";

export default function LGAForgotPasswordPage() {
  ...
  const onSubmit = async (data: ForgotPasswordInput) => {
    setLoading(true);
    try {
      const res = await fetch("/api/lga/forgot-password", {
        method: "POST",
              ...
              <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                ...
                <div className="p-8">
                  ...
                  <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-green-100 border border-green-200 text-green-800 text-[11px] font-bold uppercase tracking-widest mb-4">
                    <BadgeCheck className="h-3 w-3" />LGA Account Recovery
                  </div>
                  <h1 className="text-2xl font-bold text-slate-900 mb-2">Reset LGA Password</h1>
                  <p className="text-sm text-slate-500 mb-6">
                    Enter the official email address for your LGA administrator account.
                  </p>

                  <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                    <Input
                      label="LGA Administrator Email"
                      type="email"
