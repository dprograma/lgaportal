...
function ResetPasswordContent() {
  ...
  const onSubmit = async (data: ResetPasswordInput) => {
    ...
    try {
      ...
      if (!res.ok) {
        toast.error(json.error ?? "Failed to reset password. The link may have expired.");
        return;
      }
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <div className="p-8">
              ...
              <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-green-100 border border-green-200 text-green-800 text-[11px] font-bold uppercase tracking-widest mb-4">
                <BadgeCheck className="h-3 w-3" />Password Reset
              </div>
              <h1 className="text-2xl font-bold text-slate-900 mb-2">Set a New Password</h1>
              <p className="text-sm text-slate-500 mb-6">
                Create a strong password for your citizen account.
              </p>
