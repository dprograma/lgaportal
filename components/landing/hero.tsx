...
interface ActivityData {
  feed: { text: string; time: string; href: string }[];
            ...
            <div className="relative w-full max-w-sm">
              ...
              <div className="bg-white rounded-2xl border border-slate-200 shadow-elevated p-5 relative">
                ...
                <div className="space-y-3 min-h-[80px]">
                  {feed.length > 0
                    ? feed.map((item, i) => (
                        <Link key={i} href={item.href} className="flex items-start gap-3 no-underline group">
                          <div className="h-1.5 w-1.5 rounded-full bg-green-500 mt-1.5 shrink-0" />
                          <div className="flex-1 min-w-0">
                            <p className="text-xs text-slate-700 font-medium leading-snug line-clamp-2 group-hover:text-green-700 transition-colors">{item.text}</p>
